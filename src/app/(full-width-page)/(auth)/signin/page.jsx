...
export default function page() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res?.status === 200) {
        toast.success(res.data.message);
        dispache(setUser(res.data.user.email))
      } else {
        toast.warning(res.data.message);
      }
    } catch (error) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      ...
      <div className="bg-white shadow-2xl rounded-xl grid grid-cols-1 md:grid-cols-2 max-w-6xl w-full overflow-hidden">
        ...
        <div className="p-10 relative">
          ...
          <h2 className="text-3xl font-bold text-center mt-10">Sign In</h2>
          ...
          <form className="mt-6 space-y-4">
            ...
            <input type="email" name="email" onChange={handleChange} placeholder="Email" className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400" />
            ...
            <button onClick={handleSubmit} className="w-full bg-blue-600 text-white py-2 rounded-lg font-semibold hover:bg-blue-700 transition">Log In</button>
          </form>

          <p className="text-center text-sm mt-4 text-gray-500">
            Create an account? <Link href={"/signup"} className="text-blue-600 hover:underline" >Sign Up</Link>
          </p>
        </div>
